...
uint32_t   WrittenToRom=0, MemoryState=0;
...
{
    return (ROMPages[addr / 0x10000]) ? ROMPages[addr / 0x10000] +
	(addr % 0x10000) : &EmptySpace;
}
...
int32_t Allocate_Memory(void)
{
    ...
    MemChunk = (uint8_t*)
	mmap(NULL, 0x100000 * sizeof(uintptr_t) + 0x1D000 + RdramSize,
    ...
    NOMEM = (uint8_t*)
	mmap((void *) ((uintptr_t) N64MEM + RdramSize), 0xD000, PROT_NONE,
	     MAP_PRIVATE | MAP_ANONYMOUS | MAP_FIXED, 0, 0);
    ...
    Registers =
	(N64_REGISTERS *) ((uintptr_t) MemChunk +
...
uint32_t r4300i_LD_VAddr(uint32_t VAddr, uint64_t * Value)
{
    ...
    *((uint32_t *) (Value) + 1) =
	*(uint32_t *) (TLB_Map[VAddr >> 12] + VAddr);
    *((uint32_t *) (Value)) =
	*(uint32_t *) (TLB_Map[VAddr >> 12] + VAddr + 4);
    return 1;
}

int32_t r4300i_LH_NonMemory(uint32_t PAddr, uint32_t * Value,
			    int32_t SignExtend)
{
    switch (PAddr & 0xFFF00000) {
    default:
...
int r4300i_CPU_MemoryFilter64_2(uintptr_t MemAddress,
				ucontext_t * context);
...
void sig_handler(int signo, siginfo_t * info, ucontext_t * context)
{
	 ...
	 */
#ifdef USEX64
	if ((uintptr_t)context->uc_mcontext.gregs[REG_R15] != (uintptr_t) TLB_Map) {
