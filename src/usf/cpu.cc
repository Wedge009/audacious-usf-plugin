...
void ReadFromMem(const void *source, void *target, uint32_t length,
		 uint32_t * offset)
{
...
uint32_t Machine_LoadStateFromRAM(void *savestatespace)
{
    ...
    ReadFromMem(savestatespace, &SaveRDRAMSize, sizeof(SaveRDRAMSize),
		&offset);
    ...
    ReadFromMem(savestatespace, &PROGRAM_COUNTER, sizeof(PROGRAM_COUNTER),
		&offset);
...
void controlfp(uint32_t control)
{
    ...
    OldControl &=
	~(_FPU_RC_ZERO | _FPU_RC_UP | _FPU_RC_DOWN | _FPU_RC_NEAREST);
