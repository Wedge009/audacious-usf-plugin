...
void r4300i_BEQ(void)
{
	...
	JumpToLocation =
	    PROGRAM_COUNTER + ((int16_t) Opcode.offset << 2) + 4;
	TestInterpreterJump(PROGRAM_COUNTER, JumpToLocation, Opcode.rs,
			    Opcode.rt);
    } else {
...
void r4300i_ADDI(void)
{
    ...
    GPR[Opcode.rt].DW =
	(GPR[Opcode.rs].W[0] + ((int16_t) Opcode.immediate));
}
...
{
    GPR[Opcode.rt].DW =
	GPR[Opcode.rs].DW + (int64_t) ((int16_t) Opcode.immediate);
}
...
{
    uint32_t Address =
	GPR[Opcode.base].UW[0] + (uint32_t) ((int16_t) Opcode.offset);
    TEST_COP1_USABLE_EXCEPTION if ((Address & 3) != 0) {
	ADDRESS_ERROR_EXCEPTION(Address, 1);
    }
    if (!r4300i_LW_VAddr
	(Address, &*(uint32_t *) FPRFloatLocation[Opcode.ft])) {
	//if (ShowTLBMisses) {
...
void r4300i_LDC1(void)
{
    ...
    TEST_COP1_USABLE_EXCEPTION if ((Address & 7) != 0) {
	ADDRESS_ERROR_EXCEPTION(Address, 1);
    }
    if (!r4300i_LD_VAddr
	(Address, &*(uint64_t *) FPRDoubleLocation[Opcode.ft])) {
    }
}
...
void r4300i_SDC1(void)
{
    ...
    if (!r4300i_SD_VAddr
	(Address, *(int64_t *) FPRDoubleLocation[Opcode.ft])) {
    }
}
...
void r4300i_SPECIAL_SLLV(void)
{
    ...
    GPR[Opcode.rd].DW =
	(GPR[Opcode.rt].W[0] << (GPR[Opcode.rs].UW[0] & 0x1F));
}

void r4300i_SPECIAL_SRLV(void)
{
    GPR[Opcode.rd].DW =
	(int32_t) (GPR[Opcode.rt].UW[0] >> (GPR[Opcode.rs].UW[0] & 0x1F));
}
...
{
    GPR[Opcode.rd].UDW =
	GPR[Opcode.rt].UDW >> (GPR[Opcode.rs].UW[0] & 0x3F);
}
...
{
    HI.DW =
	(int64_t) (GPR[Opcode.rs].W[0]) * (int64_t) (GPR[Opcode.rt].W[0]);
    LO.DW = HI.W[0];
    HI.DW = HI.W[1];
}
...
{
    HI.DW =
	(uint64_t) (GPR[Opcode.rs].UW[0]) *
	(uint64_t) (GPR[Opcode.rt].UW[0]);
    LO.DW = HI.W[0];
    HI.DW = HI.W[1];
}
...
void r4300i_SPECIAL_DMULT(void)
{
    ...
    LO.UDW =
	(uint64_t) GPR[Opcode.rs].UW[0] * (uint64_t) GPR[Opcode.rt].UW[0];
    Tmp[0].UDW =
	(int64_t) GPR[Opcode.rs].W[1] *
	(int64_t) (uint64_t) GPR[Opcode.rt].UW[0];
    Tmp[1].UDW =
	(int64_t) (uint64_t) GPR[Opcode.rs].UW[0] *
	(int64_t) GPR[Opcode.rt].W[1];
    HI.UDW = (int64_t) GPR[Opcode.rs].W[1] * (int64_t) GPR[Opcode.rt].W[1];

    Tmp[2].UDW =
	(uint64_t) LO.UW[1] + (uint64_t) Tmp[0].UW[0] +
	(uint64_t) Tmp[1].UW[0];
    LO.UDW += ((uint64_t) Tmp[0].UW[0] + (uint64_t) Tmp[1].UW[0]) << 32;
    HI.UDW +=
	(uint64_t) Tmp[0].W[1] + (uint64_t) Tmp[1].W[1] + Tmp[2].UW[1];
}
...
void r4300i_SPECIAL_DMULTU(void)
{
	...
	(uint64_t) GPR[Opcode.rs].UW[0] * (uint64_t) GPR[Opcode.rt].UW[0];
    Tmp[0].UDW =
	(uint64_t) GPR[Opcode.rs].UW[1] * (uint64_t) GPR[Opcode.rt].UW[0];
    Tmp[1].UDW =
	(uint64_t) GPR[Opcode.rs].UW[0] * (uint64_t) GPR[Opcode.rt].UW[1];
    HI.UDW =
	(uint64_t) GPR[Opcode.rs].UW[1] * (uint64_t) GPR[Opcode.rt].UW[1];
    ...
    LO.UDW += ((uint64_t) Tmp[0].UW[0] + (uint64_t) Tmp[1].UW[0]) << 32;
    HI.UDW +=
	(uint64_t) Tmp[0].UW[1] + (uint64_t) Tmp[1].UW[1] + Tmp[2].UW[1];
}
...
{
    TEST_COP1_USABLE_EXCEPTION if (Opcode.fs != 31 && Opcode.fs != 0) {
	return;
    }
...
{
    TEST_COP1_USABLE_EXCEPTION if (Opcode.fs == 31) {
...
{
    TEST_COP1_USABLE_EXCEPTION NextInstruction = DELAY_SLOT;
...
{
    TEST_COP1_USABLE_EXCEPTION if ((FPCR[31] & FPCSR_C) == 0) {
	NextInstruction = DELAY_SLOT;
	JumpToLocation =
	    PROGRAM_COUNTER + ((int16_t) Opcode.offset << 2) + 4;
    } else {
...
{
    TEST_COP1_USABLE_EXCEPTION controlfp(RoundingModel);
    *(float *) FPRFloatLocation[Opcode.fd] =
	(*(float *) FPRFloatLocation[Opcode.fs] +
	 *(float *) FPRFloatLocation[Opcode.ft]);
}
...
void r4300i_COP1_S_SQRT(void)
{
    TEST_COP1_USABLE_EXCEPTION controlfp(RoundingModel);
    *(float *) FPRFloatLocation[Opcode.fd] =
	(float) sqrt(*(float *) FPRFloatLocation[Opcode.fs]);
}
...
void r4300i_COP1_S_ABS(void)
{
    TEST_COP1_USABLE_EXCEPTION controlfp(RoundingModel);
    *(float *) FPRFloatLocation[Opcode.fd] =
	(float) fabs(*(float *) FPRFloatLocation[Opcode.fs]);
}
...
void r4300i_COP1_S_NEG(void)
{
    TEST_COP1_USABLE_EXCEPTION controlfp(RoundingModel);
    *(float *) FPRFloatLocation[Opcode.fd] =
	(*(float *) FPRFloatLocation[Opcode.fs] * -1.0f);
}
...
{
    TEST_COP1_USABLE_EXCEPTION controlfp(RoundingModel);
    *(double *) FPRDoubleLocation[Opcode.fd] =
	(double) (*(float *) FPRFloatLocation[Opcode.fs]);
}
...
void r4300i_COP1_S_CMP(void)
{
    ...
    condition = ((Opcode.funct & 4) && less) | ((Opcode.funct & 2)
						&& equal) | ((Opcode.
							      funct & 1)
							     && unorded);
...
void r4300i_COP1_D_ADD(void)
{
    TEST_COP1_USABLE_EXCEPTION
	* (double *) FPRDoubleLocation[Opcode.fd] =
	*(double *) FPRDoubleLocation[Opcode.fs] +
	*(double *) FPRDoubleLocation[Opcode.ft];
}
...
void r4300i_COP1_D_SQRT(void)
{
    TEST_COP1_USABLE_EXCEPTION
	* (double *) FPRDoubleLocation[Opcode.fd] =
	(double) sqrt(*(double *) FPRDoubleLocation[Opcode.fs]);
}
...
void r4300i_COP1_D_ABS(void)
{
    TEST_COP1_USABLE_EXCEPTION
	* (double *) FPRDoubleLocation[Opcode.fd] =
	fabs(*(double *) FPRDoubleLocation[Opcode.fs]);
}

void r4300i_COP1_D_MOV(void)
{
    TEST_COP1_USABLE_EXCEPTION
	* (int64_t *) FPRDoubleLocation[Opcode.fd] =
	*(int64_t *) FPRDoubleLocation[Opcode.fs];
}
...
void r4300i_COP1_D_NEG(void)
{
    TEST_COP1_USABLE_EXCEPTION
	* (double *) FPRDoubleLocation[Opcode.fd] =
	(*(double *) FPRDoubleLocation[Opcode.fs] * -1.0);
}

void r4300i_COP1_D_TRUNC_L(void)
{				//added by Witten
    TEST_COP1_USABLE_EXCEPTION controlfp(_FPU_RC_ZERO);
    Double_RoundToInteger64(&*(int64_t *) FPRFloatLocation[Opcode.fd],
			    &*(double *) FPRDoubleLocation[Opcode.fs]);
}

void r4300i_COP1_D_CEIL_L(void)
{				//added by Witten
    TEST_COP1_USABLE_EXCEPTION controlfp(_FPU_RC_UP);
    Double_RoundToInteger64(&*(int64_t *) FPRFloatLocation[Opcode.fd],
			    &*(double *) FPRDoubleLocation[Opcode.fs]);
}

void r4300i_COP1_D_FLOOR_L(void)
{				//added by Witten
    TEST_COP1_USABLE_EXCEPTION controlfp(_FPU_RC_DOWN);
    Double_RoundToInteger64(&*(int64_t *) FPRDoubleLocation[Opcode.fd],
			    &*(double *) FPRFloatLocation[Opcode.fs]);
}

void r4300i_COP1_D_ROUND_W(void)
{
    TEST_COP1_USABLE_EXCEPTION controlfp(_FPU_RC_NEAREST);
    Double_RoundToInteger32(&*(int32_t *) FPRFloatLocation[Opcode.fd],
			    &*(double *) FPRDoubleLocation[Opcode.fs]);
}
...
void r4300i_COP1_D_CEIL_W(void)
{				//added by Witten
    TEST_COP1_USABLE_EXCEPTION controlfp(_FPU_RC_UP);
    Double_RoundToInteger32(&*(int32_t *) FPRFloatLocation[Opcode.fd],
			    &*(double *) FPRDoubleLocation[Opcode.fs]);
...
void r4300i_COP1_D_FLOOR_W(void)
{				//added by Witten
    TEST_COP1_USABLE_EXCEPTION controlfp(_FPU_RC_DOWN);
    Double_RoundToInteger32(&*(int32_t *) FPRDoubleLocation[Opcode.fd],
			    &*(double *) FPRFloatLocation[Opcode.fs]);
}
...
void r4300i_COP1_D_CVT_S(void)
{
    TEST_COP1_USABLE_EXCEPTION controlfp(RoundingModel);
    *(float *) FPRFloatLocation[Opcode.fd] =
	(float) *(double *) FPRDoubleLocation[Opcode.fs];
}
...
void r4300i_COP1_W_CVT_S(void)
{
    TEST_COP1_USABLE_EXCEPTION controlfp(RoundingModel);
    *(float *) FPRFloatLocation[Opcode.fd] =
	(float) *(int32_t *) FPRFloatLocation[Opcode.fs];
}
...
void r4300i_COP1_W_CVT_D(void)
{
    TEST_COP1_USABLE_EXCEPTION controlfp(RoundingModel);
    *(double *) FPRDoubleLocation[Opcode.fd] =
	(double) *(int32_t *) FPRFloatLocation[Opcode.fs];
}
...
void r4300i_COP1_L_CVT_S(void)
{
    TEST_COP1_USABLE_EXCEPTION controlfp(RoundingModel);
    *(float *) FPRFloatLocation[Opcode.fd] =
	(float) *(int64_t *) FPRDoubleLocation[Opcode.fs];
}
...
void r4300i_COP1_L_CVT_D(void)
{
    TEST_COP1_USABLE_EXCEPTION controlfp(RoundingModel);
    *(double *) FPRDoubleLocation[Opcode.fd] =
	(double) *(int64_t *) FPRDoubleLocation[Opcode.fs];
}
